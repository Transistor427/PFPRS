...
### Подготовка
1) Запускаем модифицированный G-код с макросом `_LOG_Z`
...
---
Обратите внимание, что даже если визуально кажется, что стол не опустился, в реальности он опустился вниз на 0.1-0.3мм, поэтому лучше всего поднять стол как раз на эти 0.1-0.3 мм (возможно более менее универсальным значением будет поднимать стол на высоту слоя печати. В этом случае получится избежать пробелов между слоями, однако сопло может хорошенько потереться об уже напечатанный слой) ближе к соплу, после чего восстановить печать.
...
## Алгоритм восстановления печати после длительного отключения питания

Условия выполнения восстановления:
- пропал свет на несколько часов/на ночь
- деталь **НЕ** отлипла от стола

### Подготовка
Печатается модифицированный G-код с макросом `_LOG_Z`

### Восстановление через KlipperScreen
1) Включаем принтер, если еще не включен
2) Включаем нагрев стола до целевой температуры 
3) Включаем хотенд и дожидаемся нагрева до температуры размягчения используемого филамента
4) Меню `"Движение"` > меню `"Действия"` > панель `"Аварийные перемещения"` > кнопка `"Установить Z в максимум"` > поднимаем стол вверх примерно на высоту слоя (если слой 0.2мм, то поднимаем стол на 0.2), если видно что он чуть-чуть сместился или если опустился слишком сильно, то поднимаем, пока сопло не будет на той же высоте, что и при печати > кнопка `"Установить позицию Z в 0"` > кнопка `"Возобновить"` (иконка с кругом в который вписан треугольник) 
5) 
...
```
#### Удаляем импорт в `printer.cfg`:
Веб-интерфейс > Конфигурация > printer.cfg > Удаляем строчку:
```
[include klipper-config/pfprs/pfprs.cfg]
```
#### Удаляем папку из домашней директории:
```
sudo rm -rf ~/PFPRS
```
## Обновление
1) Проводим удаление из предущего пункта
2) Проводим новую установку
